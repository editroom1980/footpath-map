...
| `routeTurns` | 曲がり角に置く進行方向の三角（v111）。**表示専用** |
...
routeLine.setLatLngs(...)           ← 画面に出る赤い破線
   ...
   └ routeTurns                     ★表示専用：曲がり角に小さな三角（v111）
```
...
- **進行方向は「曲がり角だけ」に小さな三角で示す**（v111、`routeTurns`／`_buildTurnMarks`）。
  線の全体に矢印を並べる方式は v108→v109 で試して読みにくく、v110 で取り下げた。
  角の判定は**線に沿って前後 22px の向きの差**で行う（隣り合う点の角度では、点が細かい
  OSRMのルートでほとんど検出できない）。三角は角の少し先に置く（角がスポットだと
  ○の下に隠れるため）。表示専用の約束は `routeCasing` と同じ。
