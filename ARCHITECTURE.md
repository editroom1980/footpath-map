...
2. それ以外は **OSRM**（`router.project-osrm.org` の徒歩プロファイル）へ問い合わせ、結果を使う
3. 失敗したら**直線**にし、`_warnRouteFallback()` で利用者に知らせる（30秒に1回まで）
4. 手動モード（両端の fit が false）は最初から直線

同じ区間の結果は `segCache` に貯めて再利用する。

> **OSRM は公開デモサーバ**で「1秒1回まで・非商用・予告なく停止しうる」と明記されている。
> 止まると全区間が直線になる。**残っている最大の弱点**（待避策は未実装）。
...
| 場所 | 例 |
|---|---|
| `index.html` の `APP_VERSION` | `'v92'` |
| `version.json` | `{"version":"v92"}` |
| `package.json` の `version` | `0.92.0` |
...
`footpath_regression.py`（**184項目**）が本書の条件を機械で見張っている。
