...
最終更新時点の版：**v85** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**112項目**）。
...
- **[中] タブを閉じる/再読込で未保存の変更が消える**（`beforeunload` なし。→提案12）。
- **[中] オフライン不可**：現地（圏外）で地図が出ない（→提案5）。
- **[低] 所要時間が歩行4km/h固定・坂を考慮しない**（標高データは取得済み。→提案7）。
