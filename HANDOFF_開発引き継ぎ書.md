...
最終更新時点の版：**v90** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**166項目**（うちオフライン検査5項目はローカルHTTP配信＋圏外エミュレーションで実行））。
