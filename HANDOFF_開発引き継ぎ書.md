...
最終更新時点の版：**v88** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**135項目**）。
...
- **[高] 写真が localStorage(約5MB)を圧迫**：写真は data URI でコースJSONに埋め込み。v81 で超過時に警告は出るが根本解決ではない（→提案8）。
...
8. 写真を IndexedDB へ退避（容量の根本対策）… 中／大
