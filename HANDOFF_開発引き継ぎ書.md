...
最終更新時点の版：**v87** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**131項目**）。
...
- 文字サイズ設定は項目名自体をその大きさで表示（メニューが地図を覆うため比較できない）
- コース0件時の画面を案内に（作成 or サンプルへ誘導）
