...
最終更新時点の版：**v81** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**78項目**）。
...
- **その他**：地名ラベルサイズの恒久化は **v80 で対応済み**（`LS.labelSize`/`LS.wpSize`・`restoreSizePrefs()`）。○内文字の縦中心は html2canvas で約+1%（ほぼ中央、必要なら微調整）。
