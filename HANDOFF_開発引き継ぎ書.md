...
最終更新時点の版：**v82** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**89項目**）。
...
- **[低] 全コース一括バックアップが無い**（→提案3）。
- **[低] `user-scalable=no`** でブラウザ拡大ができない（見えづらい人に不利）。

**提案（効果／手間）**
1. 誰でも開ける公開リンク（`?course=xxx.json` を同じ場所に配置。QR化も可）… 大／小
2. GPX 書き出し・読み込み（YAMAP・ヤマレコ・Garmin・Google Earth と相互運用）… 大／小
3. 全コース一括バックアップ／復元 … 中／小
