...
最終更新時点の版：**v86** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**125項目**）。
...
**提案（効果／手間）**
1. ~~誰でも開ける公開リンク~~ → **v82 で実施済み**（QRコード化は未対応：CDN追加が要るため要相談）
