...
最終更新時点の版：**v91** ／ 対象：`index.html`（単一ファイル）
...
- 実行：`PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers python3 footpath_regression.py /path/to/index.html`（終了コード0＝全PASS。項目数は実行時に表示、現在**173項目**（うちオフライン検査5項目はローカルHTTP配信＋圏外エミュレーションで実行））。
...
- **v91** **動作確認（実機チェック）画面**。開発者ツール無しで、実機で「本当に動いているか」を調べられる。版・ネット接続・部品(Leaflet/html2canvas/QR)の読込・保存領域・写真の保存先・サービスワーカーの状態・保存タイル数・画面/タッチ/GPS権限、さらに**地図タイル／経路サーバ／標高APIへ実際に1回ずつ接続して応答時間まで測る**。入口は 起動画面の「動作確認（実機チェック）」リンク・スマホメニュー・`?check=1`。「結果をコピー」でテキスト化でき、そのまま開発側へ送れる。圏外確認の手順も画面内に記載。
